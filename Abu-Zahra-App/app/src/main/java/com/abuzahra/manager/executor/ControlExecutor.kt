...
object ControlExecutor {
    ...
    fun takeScreenshot(context: Context): Map<String, Any> {
        return try {
            ...
            mapOf("error" to (e.message ?: "Screenshot failed") as Any)
        }
    }
    ...
    private fun takePhoto(context: Context, facing: Int): Map<String, Any> {
        return try {
            ...
            mapOf("error" to (e.message ?: "Camera failed") as Any)
        }
    }
    ...
    fun airplaneOn(context: Context): String {
        return try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
                // On Android 4.3+, this requires WRITE_SECURE_SETTINGS permission
                // which is only granted to system apps
    ...
    fun speakText(context: Context, params: Map<String, Any>): String {
        ...
        return if (text.isNotBlank()) {
            try {
                val ttsRef = arrayOfNulls<android.speech.tts.TextToSpeech>(1)
                ttsRef[0] = android.speech.tts.TextToSpeech(context) { status ->
                    if (status == android.speech.tts.TextToSpeech.SUCCESS) {
                        ttsRef[0]?.speak(text, android.speech.tts.TextToSpeech.QUEUE_FLUSH, null, "tts1")
                    }
                }
                "Speaking: $text"
            } catch (e: Exception) {
